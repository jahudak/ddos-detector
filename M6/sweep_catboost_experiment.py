...
def merge_events_components(events_df, components_df):
    ...
    # Merge on 'Attack ID'
    merged_df = pd.merge(components_df, events_df_clean, on="Attack ID", how="inner")
...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[(df["Start time"] != "0") & (df["End time"] != "0")]
    ...
    df["Start_time_hour_sin"], df["Start_time_hour_cos"] = sin_cos_encode(
        df["Start_time_hour"], 24
    )
    df["Start_time_weekday_sin"], df["Start_time_weekday_cos"] = sin_cos_encode(
        df["Start_time_weekday"], 7
    )
    df["Start_time_dayofyear_sin"], df["Start_time_dayofyear_cos"] = sin_cos_encode(
        df["Start_time_dayofyear"], 365
    )

    df["End_time_hour_sin"], df["End_time_hour_cos"] = sin_cos_encode(
        df["End_time_hour"], 24
    )
    df["End_time_weekday_sin"], df["End_time_weekday_cos"] = sin_cos_encode(
        df["End_time_weekday"], 7
    )
    df["End_time_dayofyear_sin"], df["End_time_dayofyear_cos"] = sin_cos_encode(
        df["End_time_dayofyear"], 365
    )
    ...
    df["type"] = df["type"].map(
        {"type_ddos": "ddos", "type_sus": "sus", "type_normal": "normal"}
    )
...
sweep_config = {
    ...
    "parameters": {
        # "iterations": {"values": [100, 300, 500]},
        # "learning_rate": {"min": 0.01, "max": 0.2},
        # "depth": {"values": [4, 6, 8, 10]},
        # "l2_leaf_reg": {"min": 1, "max": 10},
        # "bagging_temperature": {"min": 0.0, "max": 1.0},
        # "random_strength": {"min": 0, "max": 10},
        # og fent
        "iterations": {"values": [100, 300, 500, 800, 1000]},
        "learning_rate": {"min": 0.005, "max": 0.2},
        "depth": {"values": [4, 6, 8, 10]},
        "l2_leaf_reg": {"min": 1, "max": 20},
        "bagging_temperature": {"min": 0.0, "max": 1.0},
        "random_strength": {"min": 0, "max": 30},
        "rsm": {"min": 0.5, "max": 1.0},  # Random Subspace Method
    },
}
...
def sweep(df: pd.DataFrame = df, df_eval: pd.DataFrame = df_eval) -> dict:
    ...
    # normalized_weights = {
    #     k: (inverse_class_weights[k] / total_weight) * len(classes)
    #     for k in inverse_class_weights
    # }
    ...
    clf = CatBoostClassifier(
        ...
        loss_function="MultiClassOneVsAll",  # egy nehezen megtalálható osztályt szed elő és arra optimalizál
        verbose=50,
        random_state=42,
        od_type="Iter",
        od_wait=20,
    ...
    wandb.log({"f1_macro": f1, "roc_auc_macro": roc_auc, "pr_auc_macro": pr_auc})

    # Call the plot_results function
    # plot_results(y_eval, y_pred, classes)

    wandb.finish()

    return {"f1_macro": f1, "roc_auc_macro": roc_auc, "pr_auc_macro": pr_auc}


print("Starting sweep...")

sweep_id = wandb.sweep(
    sweep_config, project="hamlab - MultiClassOneVsAll - no weights - extended - 30"
)
