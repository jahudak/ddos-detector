...
def eval(df_train: pd.DataFrame, df_test: pd.DataFrame, df_val: pd.DataFrame) -> dict:
    ...
    cat = CatBoostClassifier(
        iterations=300,
        learning_rate=0.14042965953897785,
        depth=6,
        l2_leaf_reg=2,
        bagging_temperature=0.6674775453701898,
        random_strength=9,
    ...
    xgb = XGBClassifier(
        ...
        n_estimators=300,
        learning_rate=0.07582069478981585,
        max_depth=4,
        reg_lambda=6,
        subsample=0.5358162601342638,
        colsample_bytree=0.9879808008064996,
    ...
    clf = VotingClassifier(
        estimators=[("catboost", cat), ("lightgbm", lgbm), ("xgboost", xgb)],
        voting="soft"
    )

    clf.fit(X_train_scaled, y_train)

    def evaluate(X_scaled, y_true_str, y_bin, class_names):
        ...
        f1_macro = f1_score(y_true_str, y_pred_str, average="macro")
        ...
        f1_per_class = f1_score(y_true_str, y_pred_str, labels=class_names, average=None)
