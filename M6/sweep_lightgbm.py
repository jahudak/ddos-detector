...
sweep_config = {
    ...
    "parameters": {
        ...
        # lgbm
        "n_estimators": {"values": [100, 300, 500, 800, 1000]},
        "learning_rate": {"min": 0.01, "max": 0.2},
        "max_depth": {"values": [-1, 4, 6]},
        "num_leaves": {"min": 20, "max": 50},
...
def sweep(df: pd.DataFrame = df, df_eval: pd.DataFrame = df_eval) -> dict:
    wandb.init(project="hamlab - lightgbm")
    config = wandb.config
...
sweep_id = wandb.sweep(sweep_config, project="hamlab - lightgbm")
