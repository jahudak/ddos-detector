...
def eval(df_train: pd.DataFrame, df_test: pd.DataFrame, df_val: pd.DataFrame) -> dict:
    ...
    lgbm = LGBMClassifier(
        n_estimators=300,
        learning_rate=0.06980545747995018,
        max_depth=6,
        num_leaves=33,
        min_child_samples=36,
        subsample=0.7878716447816506,
        colsample_bytree=0.6688848884938285,
        reg_alpha=0.48804788572380686,
        reg_lambda=0.8359319877487521,
    ...
    def evaluate(X_scaled, y_true, y_bin, class_names):
        y_pred = lgbm.predict(X_scaled).ravel()
        y_prob = lgbm.predict_proba(X_scaled)
...
def plot_confusion_matrix(
    conf_matrix, class_names, scores_dict, title="Confusion Matrix", save_path=None
):
    ...
    f1_column = np.array(
        [[f1_per_class.get(cls, 0)] for cls in class_names], dtype=np.float32
    )
    ...
    sns.heatmap(
        conf_matrix_extended,
        annot=annotations,
        fmt="",
        cmap="Blues",
        xticklabels=extended_col_names,
...
plot_confusion_matrix(
    results["C"]["confusion_matrix"],
    ["ddos", "normal", "sus"],
    results["C"],
    title="Results on C",
    save_path="images/results_c.png",
)
plot_confusion_matrix(
    results["D"]["confusion_matrix"],
    ["ddos", "normal", "sus"],
    results["D"],
    title="Results on D",
    save_path="images/results_d.png",
)
